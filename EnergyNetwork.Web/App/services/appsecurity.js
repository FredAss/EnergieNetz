...
define(["durandal/system", "durandal/app", "plugins/router", "services/routeconfig", "services/utils", "services/logger", 'services/constants'], function(system, app, router, routeconfig, utils, logger, constants) {

  var self = this;
  var postBaseUrl = constants.postBaseUrl();
  var preBaseUrl = constants.preBaseUrl();

  // Build external login url
  function externalLoginsUrl(returnUrl, generateState) {
    return postBaseUrl + "/api/account/externalLogins?returnUrl=" + (encodeURIComponent(returnUrl)) +
      "&generateState=" + (generateState ? "true" : "false");
  };

  // Build manage info url
  function manageInfoUrl(returnUrl, generateState) {
    return postBaseUrl + "/api/account/manageinfo?returnUrl=" + (encodeURIComponent(returnUrl)) +
      "&generateState=" + (generateState ? "true" : "false");
  };
  ...
  function UserInfoViewModel(name, roles, isEmailConfirmed, isActivated) {
    this.name = ko.observable(name);
    this.roles = ko.observableArray(roles);
    this.isEmailConfirmed = ko.observable(isEmailConfirmed);
    this.isActivated = ko.observable(isActivated);
  };
    ...
    $(document).on("click", "#sendConfirmationMail", function(event) {
      sendConfirmationMail()
        .then(function(jqXHR, data) {
          logger.logSuccess(language.getValue('registrationSuccessful_info'), data, null, true);
  ...
  return {
    ...
    /**
		 ...
		 */
    setAuthInfo: function(userName, roles, isEmailConfirmed, isActivated, accessToken, persistent) {
      var self = this;
      ...
      self.userInfo(new UserInfoViewModel(userName, roles, isEmailConfirmed, isActivated));
      ...
      if (isActivated == false) {
        self.showActivationWarning(true);
      }
    },
    ...
    showConfirmationWarning: function(restrictedRoute) {
      logger.showAccountWarning(restrictedRoute, "emailNotConfirmed");
      bindResendEmail();
    },

    /**
         * Displays account activation message to user
         */
    showActivationWarning: function(restrictedRoute) {
      logger.showAccountWarning(restrictedRoute, "NotActivated");
    },
    ...
    hasPermission: function(companyId) {
      var result = false;

      $.ajax(routeconfig.hasPermissionUrl, {
        type: "GET",
        data: { id: companyId },
        headers: getSecurityHeaders(),
        async: false,
    ...
    /**
     * Check if the authenticathed user is activated
     */
    isUserActivated: function() {
      var self = this;
      return self.userInfo().isActivated;
    },
    ...
    updateUser: function(data) {
      return $.ajax(routeconfig.updateUserUrl, {
        type: "POST",
        data: data,
        headers: getSecurityHeaders()
      });
    ...
    /**
		 ...
		 */
    getUsers: function() {
      return $.ajax(routeconfig.usersUrl, {
        type: "GET",
        headers: getSecurityHeaders()
      });
    },
    ...
    getRoles: function() {
      return $.ajax(routeconfig.rolesUrl, {
        type: "GET",
        headers: getSecurityHeaders()
      });
    },

    getUsersGroupedByRoles: function() {
      return $.ajax(routeconfig.usersGroupedByRolesUrl, {
        type: "GET",
        headers: getSecurityHeaders()
      });
    },
    ...
    /**
		 ...
		 */
    getUsersByCompanyId: function(id) {
      return $.ajax({
        data: { id: id },
        url: routeconfig.usersByCompanyIdUrl,
        type: "GET",
        headers: getSecurityHeaders()
      });
    },

    getUserManage: function() {
      return $.ajax({
        url: routeconfig.usersManageUrl,
        type: "GET",
        headers: getSecurityHeaders()
      });
    },
    ...
    changeUserData: function(data) {
      return $.ajax(routeconfig.changeUserDataUrl, {
        type: "POST",
        data: data,
        cache: false,
        headers: getSecurityHeaders()
      ...
      return system.defer(function(dfd) {
        ...
        if (sessionStorage["associatingExternalLogin"]) {
          ...
          self.getUserInfo()
            .done(function(data) {
              if (data.userName) {
                self.setAuthInfo(data.userName, data.roles, data.isEmailConfirmed, data.isActivated);
                sessionStorage["redirectTo"] = preBaseUrl + "account/manage?externalAccessToken=" + externalAccessToken + "&externalError=" + externalError;
              }
              dfd.resolve(true);
            })
            .fail(function() {
          ...
          cleanUpLocation();
          sessionStorage["redirectTo"] = preBaseUrl + "account/externalloginfailure";
          dfd.resolve(true);
        } else if (typeof (fragment.access_token) !== "undefined") {
          cleanUpLocation();
            ...
            .done(function(data) {
              if (typeof (data.userName) !== "undefined" && typeof (data.hasRegistered) !== "undefined"
                ...
                if (data.hasRegistered) {
                  ...
                  self.setAuthInfo(data.userName, data.roles, data.isEmailConfirmed, data.isActivated, fragment.access_token, false);
                  dfd.resolve(true);
                } else if (typeof (sessionStorage["loginUrl"]) !== "undefined") {
                  loginUrl = sessionStorage["loginUrl"];
                  sessionStorage.removeItem("loginUrl");
                  sessionStorage["redirectTo"] = preBaseUrl + "account/externalloginconfirmation?userName=" + data.userName +
